...
func wsfilterhandler(c *gin.Context) {
	...
	// 获取前缀
	prefixfilter, isOk := c.GetQuery("prefixfilter")
	if isOk == false || len(prefixfilter) < 1 {
		err := errors.New("prefixfilter not exist")
		log.Error(err)
		c.SecureJSON(http.StatusOK, response.Response{Code: -1, Msg: err.Error()})
		return
	}
	...
	if handler, err := wsconf.UpgraderGlobal.Upgrade(c.Writer, c.Request, httpHeader); err != nil {
		...
		wsfiltermonitor(handler, userid, winfo.TimeOut, winfo.ChanSize, winfo.Msg.Topic, prefixfilter)
	}
