...
func wsfiltermonitor(handler *websocket.Conn, userid string, timeout int, chansize int, topic string, prefixfilter string) {

	log.Debugf("user {%v} wsfiltermonitor websocket create", userid)
	// 退出时需要断开websocket连接
	defer func() {
		if err := handler.Close(); err != nil {
			log.Errorf("user {%v} close weboscket error {%v}", userid, err)
		}
		log.Debugf("user {%v} wsfiltermonitor websocket destroy", userid)
	}()
	...
	for count := 0; count < timeout; {
		...
		case natmsg := <-msgchan:
			...
			// 判断类型
			if ((uMsg.Type == msg.TaskMsg) && (taskdata.Type == msg.TaskLogMsg)) == false {
				continue
			}

			// 判断前缀
			if strings.HasPrefix(taskdata.Msg, prefixfilter) == false {
				continue
			}
