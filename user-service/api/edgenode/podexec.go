...
func podexec(c *gin.Context) {
	...
	log.Debugln(p.Commands)
	log.Debugln(len(p.Commands))
	...
	// 建立和客户端之间的长连接
	edgeReq := &request.EdgeClientPodExec{
		Namespace: namespace,
		Pod:       p.Pod,
		Container: p.Container,
		Commands:  p.Commands,
	}
	edgeUrl := fmt.Sprintf("%v?%v", setupInfo.PodExecURL, edgeReq.QueryRaw())
	...
	// 消息转发
	podExecMsgForward(userhander, edgehandler)
}
