...
func changeclienttenant(c *gin.Context) {
	...
	tidfilter := table.TenantFilterByID{
		TenantID: p.DestTenantID,
	}
	if err := database.Mdriver.DocExist("tenants", tidfilter); err != nil {
		// 检查saas的接口
		siteInfo, err := saasbackend.SDriver.GetSiteInfo(p.DestTenantID)
		if err != nil {
			// 返回tenantid不存在
	...
	if _, isOk := ptid[p.SourceTenantID]; !isOk {
		err := fmt.Errorf("clientid {%v} source tenantid {%v} not exist error", p.ClientID, p.SourceTenantID)
		log.Error(err)
		c.SecureJSON(http.StatusOK, response.Response{Code: -1, Msg: err.Error()})
		return
	}
	if _, isOk := ptid[p.DestTenantID]; isOk {
		err := fmt.Errorf("clientid {%v} dest tenantid {%v} already contain error", p.ClientID, p.DestTenantID)
		log.Error(err)
		c.SecureJSON(http.StatusOK, response.Response{Code: -1, Msg: err.Error()})
		return
	}
	...
	delete(loginstatus.TenantID, p.SourceTenantID)
	loginstatus.TenantID[p.DestTenantID] = true
	...
	delete(ptid, p.SourceTenantID)
	ptid[p.DestTenantID] = true
