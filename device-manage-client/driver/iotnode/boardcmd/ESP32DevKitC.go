...
// ESP32DevKitC 操作
type ESP32DevKitC struct {
	baudrate     int
	emprtprogram string
}

// Burn 烧写
func (driver *ESP32DevKitC) Burn(devport string, burnfile string, burncmd string, serialport *serial.Port) (string, error) {
	...
	log.Infof("ESP32DevKitC burn cmd {%v}", cmdstr)

	cmd := exec.Command("sh", "-c", cmdstr)
	stdoutStderr, err := cmd.CombinedOutput()
	log.Infof("ESP32DevKitC burn result {%v}", string(stdoutStderr))
	if err != nil {
		log.Errorf("ESP32DevKitC burn error {%v}", err)
...
// Reset 重置
func (driver *ESP32DevKitC) Reset(devport string, serialport *serial.Port) error {
	...
	if err := serialport.SetRTS(true); err != nil {
		err := fmt.Errorf("ESP32DevKitC set rts error {%v}", err)
		log.Error(err)
		return err
	}
	if err := serialport.SetDTR(false); err != nil {
		err := fmt.Errorf("ESP32DevKitC set dtr error {%v}", err)
		log.Error(err)
		return err
	}
	...
	if err := serialport.ResetInputBuffer(); err != nil {
		err := fmt.Errorf("ESP32DevKitC reset input buffer error {%v}", err)
		log.Error(err)
		return err
	}
	...
	// 关闭串口
	if err := driver.CloseSerial(serialport); err != nil {
		err := fmt.Errorf("ESP32DevKitC CloseSerial error {%v}", err)
		log.Error(err)
		return err
	}
...
// GetBaudRate 获取波特率
func (driver *ESP32DevKitC) GetBaudRate() int {
	return driver.baudrate
}

// SetBaudRate 设置波特率
func (driver *ESP32DevKitC) SetBaudRate(baudrate int) {
	driver.baudrate = baudrate
}

// OpenSerial 打开串口
func (driver *ESP32DevKitC) OpenSerial(devport string) (*serial.Port, error) {
...
// CloseSerial 关闭串口
func (driver *ESP32DevKitC) CloseSerial(serialport *serial.Port) error {
	if serialport == nil {
...
// IsOpen 判断串口是否打开
func (driver *ESP32DevKitC) IsOpen(serialport *serial.Port) bool {
	if serialport == nil {
		return false
	}
...
// ReOpen 重新
func (driver *ESP32DevKitC) ReOpen(serialport *serial.Port) error {
	if serialport == nil {
...
// SetResetCmd 设置重置命令
func (driver *ESP32DevKitC) SetResetCmd(resetcmd string) {
	return
}

// BurnEmptyProgram 烧写空程序
func (driver *ESP32DevKitC) BurnEmptyProgram(devport string, burncmd string, serialport *serial.Port) error {
...
func (driver *ESP32DevKitC) SetEmptyProgram(emptyprogram string) {
	driver.emprtprogram = emptyprogram
}

// SetWifi 设置WiFi
func (driver *ESP32DevKitC) SetWifi(ssid string, password string) {
}
