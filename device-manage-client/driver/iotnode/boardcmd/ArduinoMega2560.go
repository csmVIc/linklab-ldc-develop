...
// ArduinoMega2560 操作
type ArduinoMega2560 struct {
	baudrate     int
	emprtprogram string
}

// Burn 烧写
func (driver *ArduinoMega2560) Burn(devport string, burnfile string, burncmd string, serialport *serial.Port) (string, error) {
	...
	log.Infof("ArduinoMega2560 burn cmd {%v}", cmdstr)

	cmd := exec.Command("sh", "-c", cmdstr)
	stdoutStderr, err := cmd.CombinedOutput()
	log.Infof("ArduinoMega2560 burn result {%v}", string(stdoutStderr))
	if err != nil {
		log.Errorf("ArduinoMega2560 burn error {%v}", err)
...
// Reset 重置
func (driver *ArduinoMega2560) Reset(devport string, serialport *serial.Port) error {
	...
	if err := serialport.SetDTR(false); err != nil {
		err := fmt.Errorf("ArduinoMega2560 set dtr error {%v}", err)
		log.Error(err)
		return err
	}
	...
	if err := serialport.ResetInputBuffer(); err != nil {
		err := fmt.Errorf("ArduinoMega2560 reset input buffer error {%v}", err)
		log.Error(err)
		return err
	}
	...
	// 关闭串口
	if err := driver.CloseSerial(serialport); err != nil {
		err := fmt.Errorf("ArduinoMega2560 CloseSerial error {%v}", err)
		log.Error(err)
		return err
	}
...
// GetBaudRate 获取波特率
func (driver *ArduinoMega2560) GetBaudRate() int {
	return driver.baudrate
}

// SetBaudRate 设置波特率
func (driver *ArduinoMega2560) SetBaudRate(baudrate int) {
	driver.baudrate = baudrate
}

// OpenSerial 打开串口
func (driver *ArduinoMega2560) OpenSerial(devport string) (*serial.Port, error) {
...
// CloseSerial 关闭串口
func (driver *ArduinoMega2560) CloseSerial(serialport *serial.Port) error {
	if serialport == nil {
...
// IsOpen 判断串口是否打开
func (driver *ArduinoMega2560) IsOpen(serialport *serial.Port) bool {
	if serialport == nil {
		return false
	}
...
// ReOpen 重新
func (driver *ArduinoMega2560) ReOpen(serialport *serial.Port) error {
	if serialport == nil {
...
// SetResetCmd 设置重置命令
func (driver *ArduinoMega2560) SetResetCmd(resetcmd string) {
	return
}

// BurnEmptyProgram 烧写空程序
func (driver *ArduinoMega2560) BurnEmptyProgram(devport string, burncmd string, serialport *serial.Port) error {
...
func (driver *ArduinoMega2560) SetEmptyProgram(emptyprogram string) {
	driver.emprtprogram = emptyprogram
}

// SetWifi 设置WiFi
func (driver *ArduinoMega2560) SetWifi(ssid string, password string) {
}
