...
// Burn 烧写
func (driver *Haas100) Burn(devport string, burnfile string, burncmd string, serialport *serial.Port) (string, error) {
	...
	log.Infof("Haas100 burn cmd {%v}", cmdstr)
	...
	for {
		...
		log.Debugf("Haas100 burn info {%v}", line)
		stdoutStderr += line
	}

	if err := cmd.Wait(); err != nil {
		err = fmt.Errorf("Haas100 burn error {%v}", err)
		log.Error(err)
		return fmt.Sprintf("%s\n\n%s", cmdstr, stdoutStderr), err
	}
...
// Reset 重置
func (driver *Haas100) Reset(devport string, serialport *serial.Port) error {
	...
	if err := serialport.ResetInputBuffer(); err != nil {
		err := fmt.Errorf("Haas100 reset input buffer error {%v}", err)
		log.Error(err)
		return err
	}
	...
	// 关闭串口
	if err := driver.CloseSerial(serialport); err != nil {
		err := fmt.Errorf("Haas100 CloseSerial error {%v}", err)
		log.Error(err)
		return err
	}
...
// GetBaudRate 获取波特率
func (driver *Haas100) GetBaudRate() int {
	return driver.baudrate
}

// SetBaudRate 设置波特率
func (driver *Haas100) SetBaudRate(baudrate int) {
	driver.baudrate = baudrate
}

// OpenSerial 打开串口
func (driver *Haas100) OpenSerial(devport string) (*serial.Port, error) {
...
// CloseSerial 关闭串口
func (driver *Haas100) CloseSerial(serialport *serial.Port) error {
	if serialport == nil {
...
// IsOpen 判断串口是否打开
func (driver *Haas100) IsOpen(serialport *serial.Port) bool {
	if serialport == nil {
		return false
	}
...
// ReOpen 重新
func (driver *Haas100) ReOpen(serialport *serial.Port) error {
	if serialport == nil {
...
// SetResetCmd 设置重置命令
func (driver *Haas100) SetResetCmd(resetcmd string) {
	return
}

// BurnEmptyProgram 烧写空程序
func (driver *Haas100) BurnEmptyProgram(devport string, burncmd string, serialport *serial.Port) error {
...
func (driver *Haas100) SetEmptyProgram(emptyprogram string) {
	driver.emprtprogram = emptyprogram
}

// SetWifi 设置WiFi
func (driver *Haas100) SetWifi(ssid string, password string) {
}
