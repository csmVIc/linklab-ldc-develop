...
// EDURISCV64 操作
type EDURISCV64 struct {
	baudrate     int
	emprtprogram string
}

// Burn 烧写
func (driver *EDURISCV64) Burn(devport string, burnfile string, burncmd string, serialport *serial.Port) (string, error) {
	...
	log.Infof("EDURISCV64 burn cmd {%v}", cmdstr)

	cmd := exec.Command("sh", "-c", cmdstr)
	stdoutStderr, err := cmd.CombinedOutput()
	log.Infof("EDURISCV64 burn result {%v}", string(stdoutStderr))
	if err != nil {
		log.Errorf("EDURISCV64 burn error {%v}", err)
		return fmt.Sprintf("%s\n\n%s", cmdstr, string(stdoutStderr)), err
	}

	// 增加判断错误
	if sindex := strings.Index(string(stdoutStderr), "success"); sindex < 0 {
		err = fmt.Errorf("EDURISCV64 burn error {%v}", string(stdoutStderr))
		return fmt.Sprintf("%s\n\n%s", cmdstr, string(stdoutStderr)), err
	}
...
// Reset 重置
func (driver *EDURISCV64) Reset(devport string, serialport *serial.Port) error {
	...
	if err := serialport.SetDTR(false); err != nil {
		err := fmt.Errorf("EDURISCV64 set dtr error {%v}", err)
		log.Error(err)
		return err
	}
	...
	if err := serialport.ResetInputBuffer(); err != nil {
		err := fmt.Errorf("EDURISCV64 reset input buffer error {%v}", err)
		log.Error(err)
		return err
	}
	...
	// 关闭串口
	if err := driver.CloseSerial(serialport); err != nil {
		err := fmt.Errorf("EDURISCV64 CloseSerial error {%v}", err)
		log.Error(err)
		return err
	}
...
// GetBaudRate 获取波特率
func (driver *EDURISCV64) GetBaudRate() int {
	return driver.baudrate
}

// SetBaudRate 设置波特率
func (driver *EDURISCV64) SetBaudRate(baudrate int) {
	driver.baudrate = baudrate
}

// OpenSerial 打开串口
func (driver *EDURISCV64) OpenSerial(devport string) (*serial.Port, error) {
...
// CloseSerial 关闭串口
func (driver *EDURISCV64) CloseSerial(serialport *serial.Port) error {
	if serialport == nil {
...
// IsOpen 判断串口是否打开
func (driver *EDURISCV64) IsOpen(serialport *serial.Port) bool {
	if serialport == nil {
		return false
	}
...
// ReOpen 重新
func (driver *EDURISCV64) ReOpen(serialport *serial.Port) error {
	if serialport == nil {
...
// SetResetCmd 设置重置命令
func (driver *EDURISCV64) SetResetCmd(resetcmd string) {
	return
}

// BurnEmptyProgram 烧写空程序
func (driver *EDURISCV64) BurnEmptyProgram(devport string, burncmd string, serialport *serial.Port) error {
...
func (driver *EDURISCV64) SetEmptyProgram(emptyprogram string) {
	driver.emprtprogram = emptyprogram
}

// SetWifi 设置WiFi
func (driver *EDURISCV64) SetWifi(ssid string, password string) {
}
