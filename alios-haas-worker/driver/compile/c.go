...
// cExecute 进行C编译
func (cd *Driver) cExecute(parameter *msg.CompileTask) ([]byte, string, error) {
	...
	defer func() {
		...
		if err := archiver.Unarchive("/app/workspace/alios_demo.zip", indir); err != nil {
			log.Errorf("compile.Driver.execute unarchive {%v} to {%v} error {%v}", "/app/workspace/alios_demo.zip", indir, err)
			return
		}
	}()
