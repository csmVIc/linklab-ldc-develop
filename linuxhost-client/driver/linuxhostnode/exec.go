...
// Exec 执行
func (ld *Driver) Exec(burninfo *msg.ClientBurnMsg, execzip string) (int, error) {

	// 解析开发板类型
	board, err := ld.GetBoardFromDevPort(burninfo.DeviceID)
	if err != nil {
	...
	// 设备操作加锁
	value, isOk := ld.devices.devicesMap.Load(burninfo.DeviceID)
	if isOk == false {
	...
	// 设置正在执行状态
	if err := ld.setrun(burninfo.DeviceID); err != nil {
		err = fmt.Errorf("devport {%v} set run state error {%v}", burninfo.DeviceID, err)
		log.Error(err)
		return 0, err
	}
	defer func() {
		if err := ld.setidle(burninfo.DeviceID); err != nil {
	...
	// 确定工作目录路径
	workdir := filepath.Join(ld.info.Workspace, fmt.Sprintf("%s-%v", burninfo.GroupID, burninfo.TaskIndex))
	...
	// 获取日志写入通道
	logchan, err := ld.getDeviceWriteLogChan(burninfo.DeviceID)
	if err != nil {
		err = fmt.Errorf("ld.getDeviceWriteLogChan {%v} error {%v}", burninfo.DeviceID, err)
		log.Error(err)
		return 0, err
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*time.Duration(burninfo.RunTime))
	defer cancel()

	cmdargs := strings.Fields(ld.info.Boards[board].ExecCmd)
	...
	go func() {
		for linereader.Scan() {
			...
			case <-time.After(time.Millisecond * time.Duration(ld.info.DeviceLog.LogTimeOutMill)):
				log.Errorf("devport {%v} log {%v} into logchan timeout {%vmills} error", burninfo.DeviceID, line, ld.info.DeviceLog.LogTimeOutMill)
				continue
			}
		}
		donechan <- nil
