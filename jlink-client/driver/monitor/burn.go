...
func (md *Driver) burnprocess() {
	...
	for burninfo := range md.burnchan {
		...
		for retrycount := 0; retrycount < md.info.Burn.MaxFileDownloadRetry; retrycount++ {
			...
			filePath, err = api.ADriver.FileDownload(board, burninfo.FileHash, burninfo.GroupID, burninfo.TaskIndex, "bin")
			if err == nil {
				break
			}
		...
		// 读取日志和命令交互
		istimeout, err := iotnode.IDriver.RTTView(burninfo)
		if err != nil {
			err = fmt.Errorf("rttview error {%v}", err)
			log.Error(err)
			MDriver.taskchan <- &iotnode.TaskRunEnd{
				BurnInfo: burninfo,
				TimeOut:  2,
				EndTime:  time.Now(),
			}
