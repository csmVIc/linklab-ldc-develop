...
type Driver struct {
	...
	taskchan chan *virtualnode.TaskRunEnd
}
...
func New(i *MInfo) (*Driver, error) {
	...
	md := &Driver{info: i, endrun: false, errchan: make(chan error, 1), burnchan: make(chan *msg.ClientBurnMsg, i.Chan.BurnSize), taskchan: make(chan *virtualnode.TaskRunEnd, i.Chan.TaskSize)}
	return md, nil
}
