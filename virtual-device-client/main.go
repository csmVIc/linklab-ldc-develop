...
import (
	...
	"linklab/device-control-v2/virtual-device-client/driver/monitor"
...
var (
	initInfo struct {
		...
		VirtualNodeInfo virtualnode.VInfo  `json:"virtualnode"`
...
func main() {
	var err error
	// virtualnode始化
	virtualnode.VDriver, err = virtualnode.New(&initInfo.VirtualNodeInfo)
	if err != nil {
		log.Panicf("virtualnode.New error {%s}", err)
	}
	// monitor初始化
